...
class oscillator(gym.Env):
    ...
    def __init__(self, ref = np.array([0, 0, 0, 6, 0, 0])):
        self.K = 5  # + np.random.uniform(0, 10, 1)
        self.c1 = 1.6
        self.c2 = 0.16
        self.c3 = 0.16
        self.c4 = 0.06
    ...
    def randomize(self):
        # self.c1 = 1.6 + np.random.uniform(-1, 1)
        # self.c2 = 0.16 + np.random.uniform(-0.1, 0.1)
        # self.c3 = 0.16 + np.random.uniform(-0.1, 0.1)
        # self.c4 = 0.06 + np.random.uniform(-0.03, 0.03)
        #
        self.K = 5 + np.random.uniform(-3, 3)
        self.b1 = 5 + np.random.uniform(-2, 2)

        return

    def get_parameter(self):
        return np.array([self.K, self.b1])
    ...
    def get_list_of_params(self):
        num = 3
        K = 5 + np.linspace(-2, 2, num)
        b1 = 5 + np.linspace(-1, 1, num)

        params = []
        for i in range(num):
            for j in range(num):
                params.append([K[i], b1[j]])
    ...
    def get_list_of_params(self):
        K = np.linspace(3, 7, 3)
        b1 = np.linspace(3, 7, 3)

        params = []
        for j in range(len(K)):
            for i in range(len(b1)):
                params.append([K[j], b1[i]])
