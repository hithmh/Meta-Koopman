...
class Koopman(base_Koopman):
    ...
    def _create_encoder(self, args):
        ...
        epsilon = base_distribution.sample([args['n_of_random_seeds'], tf.shape(self.mean)[0], args['pred_horizon']])
    ...
    def _create_koopman_operator(self, args):
        ...
        with tf.variable_scope('koopman', regularizer=self.l2_reg):
            self.A = tf.get_variable('A', shape=[args['latent_dim'], args['latent_dim']])
    ...
    def _create_forward_pred(self, args):
        ...
        phi_t = tf.reshape(phi_t, [-1, args['latent_dim']])
        mean_t = self.mean[:, 0]
        sigma_t = self.sigma[:, 0]

        for t in range(args['pred_horizon']-1):
            a_t = tf.reshape(self.a_input[:, t], [1, -1, args['act_dim']])
            a_t = tf.tile(a_t, [args['n_of_random_seeds'], 1, 1])
            a_t = tf.reshape(a_t, [-1, args['act_dim']])
            # phi_t = tf.matmul(phi_t, self.A) + tf.matmul(self.a_input[:, t], self.B)
            phi_t = tf.matmul(phi_t, self.A) + tf.matmul(a_t, self.B)
    ...
    def _create_optimizer(self, args):
        mean = tf.reshape(self.mean, [1, -1, args['pred_horizon'], args['latent_dim']])
        mean = tf.tile(mean, [args['n_of_random_seeds'], 1, 1, 1])
        mean = tf.reshape(mean, [-1, args['latent_dim']])
        # mean = tf.reshape(self.mean, [-1, args['latent_dim']])
        sigma = tf.reshape(self.sigma, [1, -1, args['pred_horizon'], args['latent_dim']])
        sigma = tf.tile(sigma, [args['n_of_random_seeds'], 1, 1, 1])
        sigma = tf.reshape(sigma, [-1, args['latent_dim']])
        ...
        y = tf.reshape(self.x_input[:, 1:], [1, -1, args['pred_horizon']-1, args['state_dim']])
        y = tf.tile(y, [args['n_of_random_seeds'], 1, 1, 1])
        y = tf.reshape(y, [-1,  args['pred_horizon']-1, args['state_dim']])

        self.reconstruct_loss = reconstruct_loss = tf.losses.mean_squared_error(labels=y,
                                                        predictions=self.forward_pred)
        reconstruct_pred_T_loss = tf.reduce_mean(tf.square(y[:, -1,]- self.forward_pred[:, -1]))
        ...
        self.loss =0 * forward_pred_loss + 100 * reconstruct_loss+ 0 * reconstruct_pred_T_loss - tf.stop_gradient(self.alpha) * self.entropy #+ weighted_reconstruct_loss
    ...
    def make_prediction(self, x_t, u, task_num, args):
        feed_dict = {}
        future_states = []
        feed_dict[self.shifted_x_t] = x_t[0]
