...
class Koopman(base_Koopman):
    ...
    def _create_koopman_operator(self, args):
        ...
        with tf.variable_scope('koopman', regularizer=self.l2_reg):
            self.A = tf.get_variable('A', shape=[args['state_dim'] + args['latent_dim'],
                                                 args['state_dim'] + args['latent_dim']])
            self.B = tf.get_variable('B', shape=[args['act_dim'], args['state_dim'] + args['latent_dim']])
    ...
    def _create_optimizer(self, args):
        ...
        self.loss =1* forward_pred_loss + 5 * reconstruct_loss + \
                   1 * reconstruct_pred_T_loss #+ weighted_reconstruct_loss + weighted_reconstruct_T_loss
