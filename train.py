...
from variant import *
from utils import visualize_predictions, visualize_partial_predictions
import logger
...
def train(args, env):
    ...
    replay_memory = get_replay_memory(args['alg_name'])(args, shift, scale, shift_u, scale_u, env, predict_evolution=True)
    model.set_shift_and_scale(replay_memory)
    replay_memory.save_data('./data/' + args['env_name'])
    ...
    for e in range(args['num_epochs']):
        ...
        string_to_print.extend(['validation_loss:', str(round(score_val, 4)), '|'])
        string_to_print.extend(['test_loss:', str(round(score_test, 4)), '|'])
